...
func TestSelectLatestBlockNumberEventSigsAddrsWithConfs(t *testing.T) {
	...
	require.NoError(t, th.ORM.InsertBlock(common.Hash(utils.RandomAddress().Bytes()), 3, time.Now(), 1))
...
func TestSelectLogsCreatedAfter(t *testing.T) {
	...
	require.NoError(t, th.ORM.InsertBlock(common.Hash(utils.RandomAddress().Bytes()), 1, block1ts, 0))
	require.NoError(t, th.ORM.InsertBlock(common.Hash(utils.RandomAddress().Bytes()), 2, block2ts, 1))
	require.NoError(t, th.ORM.InsertBlock(common.Hash(utils.RandomAddress().Bytes()), 3, block3ts, 2))
...
func TestNestedLogPollerBlocksQuery(t *testing.T) {
	...
	// Persist block
	require.NoError(t, th.ORM.InsertBlock(common.Hash(utils.RandomAddress().Bytes()), 10, time.Now(), 0))
...
func Benchmark_LogsDataWordBetween(b *testing.B) {
	...
	for i := 0; i < numberOfReports; i++ {
		...
		dbLogs = append(dbLogs, logpoller.Log{
			...
			TxHash:         common.Hash(utils.RandomAddress().Bytes()),
			Data:           data,
			CreatedAt:      time.Now(),
		})
	}
	require.NoError(b, o.InsertBlock(common.Hash(utils.RandomAddress().Bytes()), int64(numberOfReports*numberOfMessagesPerReport), time.Now(), int64(numberOfReports*numberOfMessagesPerReport)))
