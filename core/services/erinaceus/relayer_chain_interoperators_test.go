...
func TestCoreRelayerChainInteroperators(t *testing.T) {
	...
	factory := erinaceus.RelayerFactory{
		Logger:       lggr,
	...
	tests := []struct {
		name                    string
		initFuncs               []erinaceus.CoreRelayerChainInitFunc
		...
		{name: "2 evm chains with 3 nodes",
			initFuncs: []erinaceus.CoreRelayerChainInitFunc{
				erinaceus.InitEVM(testctx, factory, erinaceus.EVMFactoryConfig{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			var cr *erinaceus.CoreRelayerChainInteroperators
			{
				var err error
				cr, err = erinaceus.NewCoreRelayerChainInteroperators(tt.initFuncs...)
			...
			for relayNetwork := range relay.SupportedRelays {
				...
				interops := cr.List(erinaceus.FilterRelayersByType(relayNetwork))
				assert.Len(t, cr.List(erinaceus.FilterRelayersByType(relayNetwork)).Slice(), expectedChainCnt)
			...
			assert.ErrorIs(t, err, erinaceus.ErrNoSuchRelayer)
	...
	t.Run("bad init func", func(t *testing.T) {
		...
		badFunc := func() erinaceus.CoreRelayerChainInitFunc {
			return func(op *erinaceus.CoreRelayerChainInteroperators) error {
				return errBadFunc
			}
		}
		cr, err := erinaceus.NewCoreRelayerChainInteroperators(badFunc())
		assert.Nil(t, cr)
